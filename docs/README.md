...
이번 단계부터 웹 사용자는 로그인한 뒤 예약 생성과 예약 조회 기능을 사용합니다. 기존에는 예약자 이름(`name`)을 본인 확인 값으로 사용했지만, 이번 단계에서는 예약 생성과 조회를 로그인한 회원 기준으로 전환합니다.
...
| 예약 일정 변경 | `PUT` | `/reservations/{id}/schedule` | 이름으로 본인 확인 후 예약 날짜와 시간을 변경한다. |
| 예약 취소 | `POST` | `/reservations/{id}/cancellations` | 이름으로 본인 확인 후 예약 취소를 요청한다. |
...
인가 원칙: 이후 단계에서 인증된 사용자가 다른 회원의 예약에 접근하는 소유권 불일치 상황은 `403 Forbidden`으로 반환한다.
이번 단계의 인증 대상은 예약 생성과 예약 조회이며, 기존 이름 기반 변경/취소의 이름 불일치는 이전 정책대로 `404 Not Found`를 반환한다.
...
인증이 필요한 API: 예약 생성, 예약 조회
공통 처리 위치: HandlerInterceptor
테스트한 인증 실패 상황: 잘못된 로그인 정보, 세션 없는 예약 생성/조회 요청
```
