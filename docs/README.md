...
| API | 로그인, 테마, 예약 시간, 사용자 예약, 관리자 예약의 요청과 응답을 정리합니다. | [API 명세](#api-application-programming-interface-명세) |
...
| 기능 | Method | URL | 설명 |
| --- | --- | --- | --- |
| 로그인 | `POST` | `/login` | 로그인 ID와 비밀번호로 로그인한다. |
...
| 예약 일정 변경 | `PUT` | `/reservations/{id}/schedule` | 로그인 사용자의 예약 날짜와 시간을 변경한다. |
| 예약 취소 | `POST` | `/reservations/{id}/cancellations` | 로그인 사용자의 예약 취소를 요청한다. |
...
```http
POST /reservations/1/cancellations
```
