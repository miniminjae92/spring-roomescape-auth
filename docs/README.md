...
### 로그인과 인증
- [ ] 사용자는 로그인할 수 있다.
- [ ] 로그인에 성공하면 이후 요청에서 같은 사용자를 식별할 수 있다.
- [ ] 로그인에 실패하면 `401 Unauthorized`를 반환한다.
- [ ] 로그인하지 않은 사용자는 인증이 필요한 기능을 사용할 수 없다.
- [ ] 인증 로직이 컨트롤러마다 반복되지 않는다.

### 회원
- [ ] 회원은 로그인 ID, 비밀번호, 이름을 가진다.
- [ ] 존재하지 않는 로그인 ID로 로그인할 수 없다.
...
- [ ] 예약 생성 시 요청 이름이 아니라 로그인한 회원을 기준으로 예약을 만든다.
- [ ] 로그인한 사용자는 본인의 예약 목록을 조회할 수 있다.
- [ ] 로그인한 사용자는 본인 예약의 날짜와 시간을 변경할 수 있다.
- [ ] 로그인한 사용자는 본인 예약을 취소할 수 있다.
...
### Member
- [ ] 로그인 ID는 null이거나 빈 공백일 수 없다.
- [ ] 비밀번호는 null이거나 빈 공백일 수 없다.
- [ ] 이름은 null이거나 빈 공백일 수 없다.
- [ ] 이름은 2자 이상 20자 이하여야 한다.
- [ ] 이름은 완성형 한글, 영문, 공백만 허용한다.
...
### AuthServiceTest
- [ ] 로그인에 성공하면 회원 정보를 반환한다.
- [ ] 존재하지 않는 로그인 ID로 로그인하면 401 예외가 발생한다.
- [ ] 비밀번호가 일치하지 않으면 401 예외가 발생한다.
...
- [ ] 회원이 일치하지 않으면 예약을 변경할 수 없다.
- [ ] 회원이 일치하지 않으면 예약을 취소할 수 없다.
...
- [ ] 로그인 ID로 회원을 조회한다.
- [ ] 로그인 ID에 해당하는 회원이 없으면 빈 값을 반환한다.
...
### AuthApiTest
- [ ] 로그인한다.
- [ ] 존재하지 않는 로그인 ID이면 401을 반환한다.
- [ ] 비밀번호가 일치하지 않으면 401을 반환한다.

### ReservationApiTest
- [ ] 로그인 사용자는 예약을 생성한다.
- [ ] 로그인 사용자는 본인의 예약 목록을 조회할 수 있다.
- [ ] 로그인 사용자는 본인 예약의 날짜와 시간을 변경할 수 있다.
- [ ] 로그인 사용자는 본인 예약을 취소할 수 있다.
- [ ] 로그인하지 않고 예약을 생성하면 401을 반환한다.
- [ ] 로그인하지 않고 예약 목록을 조회하면 401을 반환한다.
- [ ] 로그인하지 않고 예약을 변경하면 401을 반환한다.
- [ ] 로그인하지 않고 예약을 취소하면 401을 반환한다.
